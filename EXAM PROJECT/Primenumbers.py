"""This module helps in checking if a character_change is prime or not.
Prime numbers are numbers that can only divide 1 and itself without reminder
examples are 2, 3, 5 etc"""
"""How it works
This module accepts a user input and loops through all character_change from 2 to that input
...
def is_prime_number(user_input):
    """An approach to check if a character_change is prime or not."""
    remainder_list = list()
    if user_input == 2:
        return 1
    else:
